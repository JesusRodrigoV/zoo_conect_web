...
export class AdminHabitat {
   ...
   */
  createHabitat(habitatData: Omit<Habitat, 'id' | 'isActive'>): Observable<Habitat> {
    try {
      const backendRequest = HabitatAdapter.toCreateRequest(habitatData);
      
      return this.http.post<HabitatBackendResponse>(this.habitatUrl, backendRequest).pipe(
   ...
   */
  getAllHabitats(skip: number = 0, limit: number = 10): Observable<Habitat[]> {
    return this.http.get<HabitatBackendResponse[]>(this.habitatUrl, { 
      params: { skip: skip.toString(), limit: limit.toString() } 
    }).pipe(
      map(response => HabitatAdapter.toFrontendList(response)),
      catchError(error => {
  ...
  getHabitatById(id: number): Observable<Habitat> {
    return this.http.get<HabitatBackendResponse>(`${this.habitatUrl}/${id}`).pipe(
      map(response => HabitatAdapter.toFrontend(response)),
      catchError(error => {
   ...
   */
  updateHabitat(id: number, habitatData: Partial<Omit<Habitat, 'id' | 'isActive'>>): Observable<Habitat> {
    const backendRequest = HabitatAdapter.toUpdateRequest(habitatData);
    
    return this.http.put<HabitatBackendResponse>(`${this.habitatUrl}/${id}`, backendRequest).pipe(
      map(response => HabitatAdapter.toFrontend(response)),
      catchError(error => {
  ...
  validateHabitatData(habitatData: Partial<Habitat>): boolean {
    ...
    if (habitatData.condicionesClimaticas && habitatData.condicionesClimaticas.trim().length < 5) {
      throw new Error('Las condiciones climáticas deben tener al menos 5 caracteres');
    }
  ...
  searchHabitatsByName(nombre: string): Observable<Habitat[]> {
    return this.getAllHabitats(0, 100).pipe(
      map(habitats => habitats.filter(habitat => 
