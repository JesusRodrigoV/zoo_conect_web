...
export default class TwoFactor {
  ...
  constructor() {
    ...
    if (!token) {
      this.toastService.showError('Error', 'Sesión inválida. Por favor, inicia sesión nuevamente.');
  ...
  protected onSubmit(): void {
    ...
    if (this.verifyForm.valid && this.sessionToken()) {
      ...
      this.twoFactorService
        ...
        .subscribe({
          next: (response) => {
            this.authStore.setTokens(response.access_token, response.refresh_token);
            
            this.authStore.loadProfile().then(() => {
              this.toastService.showSuccess('Bienvenido', 'Verificación 2FA exitosa');
      ...
      this.toastService.showError('Error', 'Por favor, ingresa un código válido de 6 dígitos');
    }
  }
